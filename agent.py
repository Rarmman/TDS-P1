...
def extract_json_answer(raw_text: str, real_log_url: str) -> dict:
    ...
    # BUG FIX: the model sometimes echoes "log_url" a second time, nested
    # inside the "answer" value itself (e.g. {"answer": {"state": "Assam",
    # "log_url": "..."}, "log_url": "..."}). That extra key makes the
    # "answer" fail an exact-match grader even when the real content is
    # correct. Strip any stray "log_url" out of the answer payload -- it
    # only ever belongs at the top level.
    def _strip_nested_log_url(value):
        if isinstance(value, dict):
            value.pop("log_url", None)
            for v in value.values():
                _strip_nested_log_url(v)
        elif isinstance(value, list):
            for v in value:
                _strip_nested_log_url(v)

    _strip_nested_log_url(parsed.get("answer"))
...
3. Output ONLY the JSON object the question asks for -- no prose, no markdown code
   fences. The object has exactly two top-level keys: "answer" and "log_url". Put
   a placeholder string in the top-level "log_url" field; the calling code will
   substitute the real URL.
4. Match the requested "answer" shape exactly. Never add extra keys beyond what's
   asked for. In particular, NEVER put "log_url" inside the "answer" value --
   "log_url" belongs only once, as a top-level sibling of "answer".
...
def main():
    """For local dev: `python agent.py`. Threads are already started at
    module import time above, so this just runs the server."""
